      ...
      <div className="first-container-w">
        <div className="details-card-w">
          ...
          <div className="rest-w" style={{width:'50%', color:'white', fontFamily:'DM Sans'}}>
            {myData&&<p style={{fontSize:'1.6rem', margin:'0'}}>{myData.Name}</p>}
            {myData&&<p style={{fontSize:'1.6rem', margin:'0'}}>{myData.Email}</p>}
            {myData&&<h1 style={{margin:'0'}}>{myData.UserName}</h1>}
            <div className="cats-w">
              {catData&& catData.map((cat,index)=>(
                <button style={{backgroundColor:'#9F94FF', border:'none', borderRadius:'2rem', width:'9rem', height:'2.5rem'}} key={index}>{cat}</button>
              ))}

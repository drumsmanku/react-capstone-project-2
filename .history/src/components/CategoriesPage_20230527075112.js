...
function CategoriesPage(props) {
  ...
  const [activeStates, setActiveStates] = useState(new Array(mainButtons.length).fill(false));

  
  const handleButtonState = (event) => {
    const buttonText = event.target.innerText;
    const buttonIndex = mainButtons.indexOf(event.target);
    ...
    if (selectedButton === event.target) {
      ...
      if (selectedButton) {
        const selectedIndex = mainButtons.indexOf(selectedButton);

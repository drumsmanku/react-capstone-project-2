...
function CategoriesPage(props) {
  ...
  const handleButtonState = (event) => {
    ...
    const isSelected = selectedButton === event.target;
    ...
    if (isSelected) {
      ...
      const newButton = <button key={buttonText}>{buttonText}</button>;
      setAdditionalButtons([...additionalButtons.filter((button) => button.key !== selectedButtonText), newButton]);
    }
  
    localStorage.setItem('categoryItems', event.target.innerText);
    event.target.style.border = isSelected && !active ? '4px solid green' : 'none';
  };

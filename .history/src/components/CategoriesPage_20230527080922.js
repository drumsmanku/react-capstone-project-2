...
function CategoriesPage(props) {
  ...
  const [activeStates, setActiveStates] = useState(main.map(() => false));
  
  const handleButtonState = (event) => {
    const buttonText = event.target.innerText;
    const buttonIndex = mainButtons.indexOf(buttonText);
    ...
    if (selectedButton === event.target) {
      ...
      if (selectedButton) {
        const selectedIndex = mainButtons.indexOf(selectedButton.innerText);
      ...
      newAdditionalButtons[buttonIndex] = event.target;

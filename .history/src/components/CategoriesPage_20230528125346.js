...
function CategoriesPage(props) {
  ...
  const handleButtonState = (event) => {
    ...
    if (additionalButtons.length >= 2) {
      ...
      setMinCats(<p style={{color:'#FF0000', fontFamily:'DM Sans', fontSize:'1.3rem'}}>Minimum 3 categories required</p>);
    }
  ...
  const removeButton = (event, buttonText) => {
    setAdditionalButtons(additionalButtons.filter((button) => button.key !== buttonText));
    const categoryButton = document.querySelector(`button[innerText="${buttonText}"]`);
    if (categoryButton) {
      ...
      <div className='second-container'>
        <div className='category-container'>
          {categories.map((category, index)=>(
            <button onClick={handleButtonState} style={{ background:'none',color:'white', backgroundColor:backgroundcolor[index], height:'10rem', width:'10rem', margin:'0.6rem', borderRadius:'1rem', fontSize:'1.7rem', fontFamily:'DM Sans', textAlign:'left',verticalAlign:'top',paddingTop:0,position:'relative',top:'0', backgroundImage:`url(${imgs[index]})`, backgroundRepeat:'no-repeat', backgroundPosition:'50% 80%', backgroundSize:'90%' }} key={index}>{category} </button>
          ))}

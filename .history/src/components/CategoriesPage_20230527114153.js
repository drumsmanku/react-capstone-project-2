...
const mainButtonStyling={background:'none',color:'white', backgroundColor:backgroundcolor[index], height:'10rem', width:'10rem', margin:'0.6rem', borderRadius:'1rem', fontSize:'1.7rem', fontFamily:'DM Sans', textAlign:'left',verticalAlign:'top',paddingTop:0,}
const styling={
...
function CategoriesPage(props) {
  ...
  const handleButtonState = (event) => {
    ...
    if (isSelected) {
      ...
      const newButton = <button key={buttonText} style={styling}>{buttonText}<button style={crossButtonStyling}>x</button></button>;
      ...
      <div className='second-container'>
        <div className='category-container'>
          {categories.map((category, index)=>(
            <button onClick={handleButtonState} style={mainButtonStyling} key={index}>{category} <img src={imgs[index]} alt="not found" width={140} style={{marginLeft:'0.3rem', marginTop:'1rem',}} /></button>
          ))}

...
function WeatherandNews() {
  ...
  const fetchNewsData = async () => {
    ...
    try {
      ...
      setError(null);
      const newIndex = Math.floor(Math.random() * newsData.data.articles.length);
  ...
  useEffect(() => {
    const fetchNewsDataInterval = setInterval(() => {
      fetchNewsData();
    }, 60000); // 60000 milliseconds = 1 minute
  
    // Fetch the initial news data
    fetchNewsData();
  
    // Clean up the interval when the component is unmounted
    return () => {
      clearInterval(fetchNewsDataInterval);
    };
  }, []);
            ...
            {weatherData && (
              <>
                ...
                >
                  ...
                  <p style={{ margin: "0", fontSize: "1.5rem", width:'10rem' }}>

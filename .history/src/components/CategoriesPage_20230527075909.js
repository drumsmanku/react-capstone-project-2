...
function CategoriesPage(props) {
  ...
  const [activeStates, setActiveStates] = useState(new Array(categories.length).fill(false));

  
  const handleButtonState = (event) => {
    const buttonText = event.target.innerText;
    const buttonIndex = categories.indexOf(buttonText);
    const newActiveStates = [...activeStates];
    const newAdditionalButtons = [...additionalButtons];
  
    if (selectedButton === event.target) {
      setSelectedButton(null);
      newActiveStates[buttonIndex] = false;
      newAdditionalButtons.splice(buttonIndex, 1);
      event.target.style.border = 'none';
    } else {
      if (selectedButton) {
        const selectedIndex = categories.indexOf(selectedButton.innerText);
        selectedButton.style.border = 'none';
        newActiveStates[selectedIndex] = false;
        newAdditionalButtons.splice(selectedIndex, 1);
      }
      setSelectedButton(event.target);
      newActiveStates[buttonIndex] = true;
      const newButton = <button key={buttonText}>{buttonText}</button>;
      newAdditionalButtons[buttonIndex] = newButton;
      event.target.style.border = '4px solid green';
    }
  
    setActiveStates(newActiveStates);
    setAdditionalButtons(newAdditionalButtons);

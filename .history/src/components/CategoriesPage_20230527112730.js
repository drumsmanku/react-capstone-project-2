...
function CategoriesPage(props) {
  ...
  const handleButtonState = (event) => {
    ...
    if (isSelected) {
      ...
      const newButton = <button key={buttonText} style={styling}>{buttonText}<button style={{background:'none', padding:'0', margin:'0', border:'none', fontSize:'1.2rem', width:'1rem'}}>x</button></button>;
